      ...
      return checkAndInstall('mongo', function (shell) {
         return confirm('do you want to install MongoDB?')
            .then(function confirmed () {
               ...
               shell.exec('echo "deb [ arch=amd64,arm64 ] http://repo.mongodb.org/apt/ubuntu xenial/mongodb-org/4.0 multiverse" | sudo tee /etc/apt/sources.list.d/mongodb-org-4.0.list');
               shell.exec('sudo apt-get update');
               // version 4.0.9 is the latest one with support for ubuntu 14.04
               shell.exec('sudo apt-get install -y mongodb-org=4.0.9 mongodb-org-server=4.0.9 mongodb-org-shell=4.0.9 mongodb-org-mongos=4.0.9 mongodb-org-tools=4.0.9');
