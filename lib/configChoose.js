...
function createFileContet (config) {
   var configFileContent = 'var config = ' + config + ';\n';
...
async function chooseEnvirnonment (projectPath) {

   if (!projectPath) log.critical('projectPath is undefined!');
   ...
   if (answer.environment === 'custom') {
      ...
      var configFileContent = createFileContet(config);
      // read file and return config
      fs.writeFile(configPath, configFileContent, function (err) {
