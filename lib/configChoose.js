...
async function getNetworkConfigPort (conf) {

   // TODO: just pass config
   // can be done after variables file is no longer required from environment files
   // config = config || {db: {}};

   var config = {};
   config.db = conf.db || {};
   config.port = conf.port || 3000;
   config.update = conf.update || {};
...
async function chooseEnvirnonment (configPath, defaultConfig) {
   ...
   const answer = await prompt([{
      ...
      initial: 0,
      choices: [
         { title: 'default', value: 'environment' },
         { title: 'development', value: 'environment-dev' },
         { title: 'production', value: 'environment-prod' },
         { title: 'rf-production', value: 'environment-rf-prod' },
   ...
   if (answer.environment === 'custom') {
      ...
      myMkdirSync(path.dirname(configPath)); // ensure that parent folder is there
